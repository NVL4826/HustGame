...
* **Movement Optimization (Wall Bypass)**: Mob behaviors like `SimpleSwarmBehavior` and `FleeBehavior` edit coordinates directly without passing through `CollisionManager.canMove()` to conserve CPU cycles.

...
* **Wave Survival & Transition Guard**: Level behaviors that require survival (such as `OutsideBehavior`) utilize `WaveManager` to spawn wave configuration entries and guard transitions by overriding `canTransition()`, which checks `waveManager.isFinished()` and `EntityManager.hasActiveEnemies()`.

    ...
    public void update(float delta, Camera camera) {
        if (!limitReached) {
            ...
            if (gameTime >= maxTime) {
                ...
                TimeLimitReachedEvent data = GamePools.obtain(TimeLimitReachedEvent.class);
                data.init();
                GameEvent<TimeLimitReachedEvent> event = GamePools.obtainEvent();
                event.init(EventType.TIME_LIMIT_REACHED, data);
    ...
    private class ActiveWave {
        ...
        void spawn(Camera camera) {
            ...
            for (Vector2 pos : positions) {
                ...
                GamePools.free(pos); // Free Vector2 back to pool after use
            }
            // Note: Strategy implementations should return a new Array or I should be careful.
            // Here they return a new Array of pooled Vector2s.
        }
    }
}

...
/**
 * AI movement behavior where the enemy pauses, shows a warning line directed towards the player,
 * and then dashes at high speed in that straight line.
 */
public class TelegraphedChargeBehavior implements AIBehavior {
    private enum State {
        CHASE,
        TELEGRAPH,
        CHARGE
    ...
    public TelegraphedChargeBehavior(final float speed) {
        this.normalSpeed = speed > 0 ? speed : 60f;
        ...
        switch (state) {
            case CHASE:
                ...
                if (dist < 300f && stateTimer <= 0f) {
                    ...
                    enemy.setTelegraphedCharging(true);
                    enemy.setTelegraphTimer(telegraphDuration);
                    
                    // The line is drawn 500 units long in that direction
                    enemy.setTelegraphTargetX(enemy.getX() + chargeDirection.x * 500f);
                    enemy.setTelegraphTargetY(enemy.getY() + chargeDirection.y * 500f);
                }
                break;

            case TELEGRAPH:
                enemy.setTelegraphTimer(stateTimer);
                if (stateTimer <= 0f) {
                    state = State.CHARGE;
            ...
            case CHARGE:
                ...
                if (stateTimer <= 0f) {
                    ...
                    enemy.setSpeed(normalSpeed);
                    enemy.setTelegraphedCharging(false);
                }
                break;
        }
    }

...
/**
 * Builder class that generates random upgrade choices for a player when leveling up.
 * Evaluates the player's current weapons and gears to determine possible upgrades.
 */
public final class LevelUpChoiceBuilder {
    ...
    /**
     * Gathers all eligible weapon and gear upgrade actions, shuffles them, and
     * returns up to 3 upgrade choices for the player.
     *
    ...
    public static Array<UpgradeAction> getLevelUpChoices(final Player player) {
        ...
        for (final String id : weaponIds) {
            ...
            if (weapon == null) {
                possibleChoices.add(new WeaponUpgradeAction(id, UpgradeCatalog.getWeaponName(id), UpgradeCatalog.getWeaponLevelDescription(id, 1), true));
            } else if (weapon.getLevel() < 5) {
                final int nextLevel = weapon.getLevel() + 1;
                possibleChoices.add(new WeaponUpgradeAction(id, UpgradeCatalog.getWeaponName(id) + " (Cấp " + nextLevel + ")", UpgradeCatalog.getWeaponLevelDescription(id, nextLevel), false));
            }
        }
        ...
        for (final String id : gearIds) {
            ...
            if (gear == null) {
                possibleChoices.add(new GearUpgradeAction(id, UpgradeCatalog.getGearName(id), UpgradeCatalog.getGearLevelDescription(id, 1), true));
            } else if (gear.getLevel() < 5) {
                final int nextLevel = gear.getLevel() + 1;
                possibleChoices.add(new GearUpgradeAction(id, UpgradeCatalog.getGearName(id) + " (Cấp " + nextLevel + ")", UpgradeCatalog.getGearLevelDescription(id, nextLevel), false));
            }
        }

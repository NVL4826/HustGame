...
/**
 * Attack behavior that periodically fires projectiles in a radial pattern (circle)
 * and rotates the angle of the circle slightly in each successive wave.
 */
public class RadialRotatingShootingBehavior implements AttackBehavior {
     ...
     */
    public RadialRotatingShootingBehavior(final float fireInterval, final float projectileSpeed, final float projectileDamage) {
    ...
    public void execute(final Enemy enemy, final float delta, final Player player, final EntityManager entityManager) {
        ...
        if (fireTimer >= currentInterval) {
            ...
            for (int i = 0; i < PROJECTILE_COUNT; i++) {
                ...
                enemy.getFactory().createProjectile(
                    enemy.getX(),
                    enemy.getY(),
                    vx,
                    vy,

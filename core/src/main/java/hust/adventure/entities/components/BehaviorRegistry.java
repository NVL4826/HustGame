...
/**
 * Unified registry mapping behavior names to suppliers for AIBehavior, AttackBehavior, and DeathBehavior,
 * returning a populated EnemyBehaviors container for a given EnemyConfig.
 */
public final class BehaviorRegistry {
    ...
    static {
        ...
        deathProviders.put("split", config -> new SplitDeathBehavior(config.getSplitType(),
                config.getSplitCount(), config.getSplitOffset()));
    }
